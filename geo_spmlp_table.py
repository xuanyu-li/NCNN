...
def get_te(layer, n_hidden, hidden_layer_conv):
    ...
    Y_tr = np.load('bgedv2_Y_tr_float64.npy')
    Y_te = np.load('bgedv2_Y_te_float64.npy')
    Y_va = np.load('bgedv2_Y_va_float64.npy')
        ...
        with torch.no_grad():
            ...
            if epoch % 5 == 0:
                ...
                if MAE_va < MAE_va_best:
                    MAE_va_best = MAE_va
                    torch.save(model.state_dict(), 'H3_geo_sparse_mlp_model_drop.pt')
                t_new = time.time()
    ...
    model.load_state_dict(torch.load('H3_geo_sparse_mlp_model_drop.pt'))
    model.eval()
    with torch.no_grad():
...
dataframe.to_csv('Geo_sparse.csv')
